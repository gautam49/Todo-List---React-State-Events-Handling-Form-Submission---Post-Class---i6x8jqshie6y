...
const Edit = ({ inputValue, index, handleEditChange, saveEdit }) => {
	return (
		<div className="edit-mode">
			<input className="editTask" type={'text'} placeholder={'edit task here...'} value={inputValue} onChange={(e) => handleEditChange(e, index)} />
			<button className="saveTask" onClick={() => saveEdit(index)}>save</button>
		</div>
	)
}
...
function App() {
	...
	const [currentValue, setCurrentValue] = useState({ edit_mode: false, value: '' });
	...
	return (
		<div id="main">
			<div>
				<input id="task" type={'text'} placeholder={'Add your todo here...'} value={currentValue.value} onChange={(e) => setCurrentValue({ ...currentValue, value: e.target.value })} />
				<button id="btn" onClick={addTodo}>Add Tod</button>
			</div>
			<div>
				<ul>
					{
						tasks.map((data, index) => {
							...
							return (
								<div key={index}>
									{
										edit_mode ?
											(<Edit inputValue={value} index={index} handleEditChange={handleEditChange} saveEdit={saveEdit} />)
											: (<div className="lists">
